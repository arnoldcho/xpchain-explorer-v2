...
module.exports = function() {
  function express_app() {
    ...
    app.get('*', hasAccess, function(req, res) {
      ...
      // Guard malformed /api/getrawtransaction requests before RPC execution.
      // This prevents noisy RPC -8 errors from empty/invalid txid values.
      if (method == 'getrawtransaction') {
        var txid = (query_parameters.txid == null ? '' : query_parameters.txid.toString().trim());

        if (!isHexTxid(txid)) {
          return res.status(400).json({
            error: 'Invalid txid. Expected a 64-character hexadecimal string.'
          });
        }
      } else if (method == 'getblock') {
        var blockhash = (query_parameters.hash == null ? '' : query_parameters.hash.toString().trim());

        if (!isHexBlockhash(blockhash)) {
          return res.status(400).json({
            error: 'Invalid block hash. Expected a 64-character hexadecimal string.'
          });
        }
      } else if (method == 'getblockhash') {
        var height = (query_parameters.height != null ? query_parameters.height : query_parameters.index);
        var parsedHeight = parseInt(height, 10);

        if (height == null || isNaN(parsedHeight) || parsedHeight < 0) {
          return res.status(400).json({
            error: 'Invalid block height. Expected a non-negative integer.'
          });
        }
      }
      ...
      client.cmd(command, function(err, response) {
        if (err) {
          // Handle expected "not found" lookups without noisy stack traces.
          if (method == 'getrawtransaction' && err.code == -5) {
            return res.status(404).json({
              error: 'Transaction not found in mempool or blockchain.',
              txid: (query_parameters.txid == null ? '' : query_parameters.txid.toString().trim())
            });
          }

          if (method == 'getblock' && err.code == -5) {
            return res.status(404).json({
              error: 'Block not found in blockchain.',
              hash: (query_parameters.hash == null ? '' : query_parameters.hash.toString().trim())
            });
          }

          console.log(err);
          res.status(500).send("There was an error. Check your console.");
        } else {
    ...
    function specialApiCase(method_name, query_parameters) {
      ...
      switch (method_name) {
        ...
        case 'getblock':
          ...
          break;
        case 'getrawtransaction':
          // Preserve RPC argument order: [txid, decrypt]
          if (query_parameters.txid != null)
            params.push(query_parameters.txid.toString().trim());

          if (query_parameters.decrypt != null) {
            var decrypt_param = parseInt(query_parameters.decrypt, 10);

            if (!isNaN(decrypt_param))
              params.push(decrypt_param);
          }

...
module.exports = {
  ...
  get_max_tx_blockindex: function(cb) {
    Tx.findOne({}, { blockindex: 1, _id: 0 }).sort({ blockindex: -1 }).then((tx) => {
      if (tx && tx.blockindex != null)
        return cb(parseInt(tx.blockindex));
      else
        return cb(0);
    }).catch((err) => {
      console.log(err);
  ...
  get_address_txs_ajax: function(hash, start, length, cb) {
    ...
    Promise.all([
      ...
      AddressTx.find({a_id: hash}).sort({blockindex: -1}).skip(skip).limit(limit).lean().exec()
    ]).then(([totalCount, balance_sum, address_tx]) => {
      var running_balance = balance_sum.length > 0 ? balance_sum[0].balance : 0;
      var txids = [...new Set(address_tx.map((row) => row.txid).filter((txid) => txid != null && txid != ''))];
      var tx_fetch = (txids.length > 0 ? Tx.find({txid: {$in: txids}}).lean().exec() : Promise.resolve([]));

      tx_fetch.then((tx_docs) => {
        var tx_map = new Map();
        var txs = [];

        tx_docs.forEach((tx) => {
          tx_map.set(tx.txid, tx);
        });

        for (var i = 0; i < address_tx.length; i++) {
          var row = address_tx[i];
          var tx = tx_map.get(row.txid);

          if (tx) {
            txs.push(Object.assign({}, tx, {balance: running_balance}));
          } else
